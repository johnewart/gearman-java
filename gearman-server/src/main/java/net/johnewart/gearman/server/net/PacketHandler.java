...
public class PacketHandler extends SimpleChannelInboundHandler<Object> {
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		LOG.trace("Client closed channel: " + ctx.channel().toString());
	...
	private void handleTextCommand(String message, Channel channel) {
		switch (message.toLowerCase()) {
			case "status" :
				...
				Map<String, JobQueue> jobQueues = networkManager.getJobManager()
					.getJobQueues();
			...
			case "workers" :
				...
				// TODO: FD is not implemented here
				ImmutableList<NetworkEngineWorker> workers = networkManager
					.getWorkerList();
				for (NetworkEngineWorker w : workers) {
					channel.writeAndFlush(String.format("%s %s %s : %s\n", 0,
						((InetSocketAddress) w.getAddress()).getAddress()
							.getHostAddress(),
						w.getId(), String.join(" ", w.getAbilities())));
				}
	...
	private void handlePacket(Packet packet, Channel channel) {
		switch (packet.getType()) {
			case CAN_DO :
				networkManager.registerAbility(
					((CanDo) packet).getFunctionName(), channel);
				return;
			case CAN_DO_TIMEOUT :
				// TODO: Support timeout
				networkManager.registerAbility(
					((CanDoTimeout) packet).getFunctionName(), channel);
				return;
			case CANT_DO :
				networkManager.unregisterAbility(
					((CantDo) packet).getFunctionName(), channel);
				return;
			case GRAB_JOB :
			...
			case OPTION_REQ :
				networkManager.handleOptionRequest((OptionRequest) packet,
					channel);
				return;
